...
#include "zdashdiamondcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDASHD ought to be enough for anybody." - Bill Gates, 2017
    ui->zDASHDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDASHDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDASHD->setEnabled(false);
        ui->pushButtonMintzDASHD->setToolTip(tr("zDASHD is currently disabled due to maintenance."));

        ui->pushButtonSpendzDASHD->setEnabled(false);
        ui->pushButtonSpendzDASHD->setToolTip(tr("zDASHD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDASHDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDASHD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDASHD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDASHD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDASHD in ") +
...
void PrivacyDialog::on_pushButtonSpendzDASHD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDASHD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDASHD
        sendzDASHD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDASHD
    sendzDASHD();
}

void PrivacyDialog::on_pushButtonZDashDiamondControl_clicked()
{
    ZDashDiamondControlDialog* zDASHDControl = new ZDashDiamondControlDialog(this);
    zDASHDControl->setModel(walletModel);
    zDASHDControl->exec();
}

void PrivacyDialog::setZDashDiamondControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDASHDSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDASHD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DashDiamond Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDASHDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDASHD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DASHDIAMOND </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDASHD</b>";
    ...
    // use mints from zDASHD selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDashDiamondControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDashDiamondControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdashdiamond selector in case it was used
    ZDashDiamondControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDASHD Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DashDiamond, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDASHD Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDASHD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDASHD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDASHD "));
    ui->labelzDASHDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDASHDSyncStatus->setVisible(fShow);
}
