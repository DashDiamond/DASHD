...
#include "ui_zdashdiamondcontroldialog.h"
...
std::list<std::string> ZDashDiamondControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDashDiamondControlDialog::listMints;

ZDashDiamondControlDialog::ZDashDiamondControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDashDiamondControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDashDiamondControlDialog::~ZDashDiamondControlDialog()
{
    delete ui;
}

void ZDashDiamondControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDashDiamondControlDialog::updateList()
{
...
void ZDashDiamondControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDashDiamondControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDashDiamond_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDashDiamondControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDashDiamondControlDialog::GetSelectedMints()
{
...
void ZDashDiamondControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
