...
int nAnonymizeDashDiamondAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "dashdiamond" is a composite category enabling all DashDiamond-related debug output
            if (ptrCategory->count(string("dashdiamond"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "dashdiamond.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "dashdiamondd.pid"));
